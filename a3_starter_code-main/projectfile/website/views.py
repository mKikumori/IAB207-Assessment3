...
def search():
    if request.args['search'] and request.args['search'] != "":
        ...
        music_shows = db.session.scalars(
            db.select(MusicShow).where(MusicShow.name.like(query)))
        return render_template('index.html', music_shows=music_shows)
    else:
...
def filter_location():
    if request.args['filter_location'] and request.args['filter_location'] != "":
        ...
        music_shows = db.session.scalars(
            db.select(MusicShow).where(MusicShow.location.like(query)))
        return render_template('index.html', music_shows=music_shows)
    else:
        return redirect(url_for('main.index'))
...
def filter_artists():
    if request.args['filter_artists'] and request.args['filter_artists'] != "":
        ...
        music_shows = db.session.scalars(
            db.select(MusicShow).where(MusicShow.artists.like(query)))
        return render_template('index.html', music_shows=music_shows)
    else:
        return redirect(url_for('main.index'))
...
def filter_genre():
    if request.args['filter_genre'] and request.args['filter_genre'] != "":
        ...
        music_shows = db.session.scalars(
            db.select(MusicShow).where(MusicShow.genre.like(query)))
        return render_template('index.html', music_shows=music_shows)
    else:
        return redirect(url_for('main.index'))
